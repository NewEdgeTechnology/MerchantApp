...
import { createOrGetOrderConversationFromOrderDetails, sendTextMessage } from "../../../utils/chatApi";
...
const AutoChatRedirect = ({
  ...
  const formatReplacementsMessage = (repls) => {
    ...
    message += "The following items were unavailable and have been replaced with similar items:\n\n";
    ...
    const totalAmount = repls.reduce((sum, r) => sum + ((r.new?.price || 0) * (r.new?.quantity || 1)), 0);
    message += `━━━━━━━━━━━━━━━━━━━━\n`;
  ...
  const handleAutoChat = async () => {
    ...
    try {
      ...
      const merchant_user_id = (await SecureStore.getItemAsync("user_id_v1")) ||
        (await SecureStore.getItemAsync("user_id")) || null;

      if (!merchant_user_id) throw new Error("Merchant user_id not found");

      const customer_id = order?.__user?.user_id ?? order?.__user?.id ??
        order?.user?.user_id ?? order?.user?.id ?? order?.user_id ??
        order?.customer_id ?? null;

      if (!customer_id) throw new Error("Customer ID not found");

      const orderIdForChat = order?.order_code || order?.order_id || order?.id || routeOrderId;
      ...
      const conversationId = resp?.conversation_id ?? resp?.data?.conversation_id ?? resp?.conversationId ?? null;
          ...
          navigation.navigate("MerchantChatRoomScreen", {
            ...
            meta: {
              ...
              customerName: order?.customer_name || order?.user_name ||
                order?.__user?.user_name || order?.__user?.name || "",
              customer_profile_image: order?.__user?.profile_image ||
                order?.__user?.profileImage || order?.__user?.avatar || "",
            },
  ...
  useEffect(() => {
    return () => { triggeredRef.current = false; };
  }, []);
