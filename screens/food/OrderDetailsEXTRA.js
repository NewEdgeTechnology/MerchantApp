...
// ✅ NEW FEATURE: Auto-chat with customer when items are replaced
// ✅ NEW FEATURE: Chat with customer button on each item
...
// ✅ Auto-chat component
import AutoChatRedirect from "./OrderDetails/AutoChatRedirect";
...
export default function OrderDetails() {
  ...
  const [pendingDeliveryChoice, setPendingDeliveryChoice] = useState(null); // 'self' or 'grab'
  const [sendingGrabRequest, setSendingGrabRequest] = useState(false);
  ...
  useEffect(() => {
    ...
    autoChatTriggeredRef.current = false; // Reset auto-chat flag
  ...
  // ✅ LOCK merchant updates AFTER driver accepts (ONLY for Grab / platform delivery)
  const isDriverAssigned = useMemo(() => {
    // For SELF delivery, driver assignment never blocks merchant updates
    if (isSelfSelected) return false;
    if (!driverAccepted) return false;
    // Only block if it's actually a platform/Grab delivery
  ...
  const shouldBlockAtReady =
    status === "READY" &&
    !isSelfSelected && // Don't block for SELF delivery
  ...
  const nextFor = useCallback(
    (curr) => {
      ...
      // Only block merchant status updates for Grab deliveries without driver
      // SELF delivery can always update status
      if (s === "READY" && shouldBlockAtReady) return null;

      // Once driver accepted for Grab delivery, merchant should not update
  ...
  const startGrabLoopForPending = useCallback(() => {
    setPendingDeliveryChoice("grab");
    setRideMessage("Tabdhey delivery selected");
  }, []);

  const stopGrabLoopForPending = useCallback(() => {
    setPendingDeliveryChoice("self");
    setRideMessage("Self delivery selected");
  }, []);

  const createBatchAndRequestDriver = useCallback(
    async (preparationMinutes) => {
      const BATCH_ENDPOINT = process.env.BATCH_ORDER_BROADCAST_ENDPOINT || "";

      if (!BATCH_ENDPOINT) {
        console.error("BATCH_ORDER_BROADCAST_ENDPOINT not configured");
        Alert.alert(
          "Configuration Error",
          "Batch endpoint not configured. Please contact support.",
        );
        return false;
      }

      try {
        setSendingGrabRequest(true);
        setRideMessage("Creating batch and finding drivers...");

        // Get order details
        const orderCode = order?.order_code || order?.id || routeOrderId;
        const customerId =
          order?.__user?.user_id ??
          order?.__user?.id ??
          order?.user_id ??
          order?.customer_id;
        const customerName =
          order?.customer_name || order?.user_name || order?.__user?.name || "";
        const customerPhone =
          order?.customer_phone || order?.__user?.phone || "";
        const deliveryAddress = order?.delivery_address?.address || "";

        // Calculate distance
        const distanceKm = computeHaversineKm(businessCoords, refCoords);
        const distanceMeters =
          distanceKm != null ? Math.round(distanceKm * 1000) : 3000;
        const estimatedDurationSecs =
          distanceKm != null ? Math.round((distanceKm / 20) * 3600) : 600;

        // Calculate delivery fee
        const deliveryFee = Number(
          order?.delivery_fee || order?.merchant_delivery_fee || 50,
        );
        const totalAmount = Number(order?.total || displayGrandTotal || 0);
        ...
        const batchPayload = {
          ...
          passenger_id: Number(merchantUserId),
          cityId: refCoords.cityId || "thimphu",
          serviceType: "Delivery",
          service_code: "D",

          pickup: [
            businessCoords?.lat || 27.4728,
            businessCoords?.lng || 89.639,
          ],
          pickup_place: order?.business_name || "Merchant Shop",

          dropoff: [refCoords.lat, refCoords.lng],
          dropoff_place: deliveryAddress || "Customer Location",
          ...
          duration_s: estimatedDurationSecs,
          preparation_time_minutes: preparationMinutes || 15,
        ...
        console.log("[OrderDetails] Sending batch request:", batchPayload);
        ...
        // Save ride ID if returned
        const newRideId = result?.ride_id || result?.data?.ride_id || null;
        if (newRideId) {
          await saveRideId(newRideId);
        }

        setRideMessage("Driver request sent! Waiting for acceptance...");
        Alert.alert(
          "Driver Request Sent",
          "Your order has been accepted and drivers are being notified.",
        );

        return true;
      } catch (error) {
        console.error("[OrderDetails] Batch creation failed:", error);
        setRideMessage("");
        Alert.alert(
          "Driver Request Failed",
          error?.message ||
            "Could not send driver request. You can try again later.",
        );
        return false;
      } finally {
        setSendingGrabRequest(false);
      }
    },
    [
      order,
  ...
  const handleAcceptWithSelectedMethod = useCallback(async () => {
    if (!pendingDeliveryChoice) {
      Alert.alert("Select Method", "Please choose a delivery method first");
      return;
    }
    ...
    if (!Number.isFinite(prepVal) || prepVal <= 0) {
      Alert.alert(
        "Time required",
        "Please enter preparation time before accepting",
      );
      return;
    }

    // Set the delivery choice
    setDeliveryChoice(pendingDeliveryChoice);

    // ✅ For GRAB: Redirect to NearbyOrdersScreen WITHOUT updating status or creating batch
    if (pendingDeliveryChoice === "grab") {
      await redirectToNearbyOrders();
      return;
    }

    // ✅ For SELF: Update order status to CONFIRMED
    await doUpdate(
      "CONFIRMED",
      { deliveryMethod: pendingDeliveryChoice },
      true,
    );
  }, [pendingDeliveryChoice, manualPrepMin, doUpdate, redirectToNearbyOrders]);
  const handleChatWithCustomer = useCallback(
    async (item) => {
      try {
        const token = await SecureStore.getItemAsync("auth_token");
        const merchant_user_id =
        ...
        if (!merchant_user_id) {
          Alert.alert("Chat", "Merchant user_id not found");
          return;
        }

        const business_id =
          businessId || paramBusinessId || order?.business_id || null;
        if (!business_id) {
          Alert.alert("Chat", "Business id not found");
          return;
        }
        ...
        if (!customer_id) {
          Alert.alert("Chat", "Customer id not found");
          return;
        }
        ...
        const orderIdForChat =
          order?.order_code || order?.order_id || order?.id || routeOrderId;
        if (!orderIdForChat) {
          Alert.alert("Chat", "Order id missing");
          return;
        }
        ...
        // Send an initial message about the unavailable item
        await sendTextMessage({
          conversationId,
          bodyText: `Hello! Unfortunately "${itemName}" is currently unavailable. Could you please:\n\n1. Suggest a similar alternative\n2. Let us know if you'd like to cancel this item\n\nPlease reply with your preference. Thank you for your understanding!`,
          userType: "MERCHANT",
          userId: merchant_user_id,
          businessIdHeader: business_id,
          token,
        });
        ...
        navigation.navigate("MerchantChatRoomScreen", {
          ...
          source: "unavailable-item-chat",
        });
      } catch (error) {
        console.error("[ChatWithCustomer] Error:", error);
        Alert.alert(
          "Chat Error",
    ...
    [navigation, order, businessId, paramBusinessId, routeOrderId],
  );
  ...
  const updateSingleStatusLikeCluster = useCallback(
    async ({
      ...
      replacementsForChat = [], // ✅ Track replacements for auto-chat
    }) => {
      ...
      try {
        ...
        // ✅ Auto-chat for replacements when accepting order
        if (
          newStatus === "CONFIRMED" &&
          replacementsForChat &&
          replacementsForChat.length > 0 &&
          !autoChatTriggeredRef.current
        ) {
          autoChatTriggeredRef.current = true;
          setPendingReplacements(replacementsForChat);
          setShowAutoChat(true);
        }

        if (
          showSuccessAlert &&
          (!replacementsForChat || replacementsForChat.length === 0)
        ) {
          Alert.alert(
            "Status updated",
        ...
        // Check if it's the wallet service error (CAPTURE_FAILED)
        if (
          errorMsg.includes("CAPTURE_FAILED") ||
          errorMsg.includes("wallet") ||
          errorMsg.includes("certificate")
        ) {
          // Still update local state even if API fails (for CAPTURE_FAILED error)
          const patch = {
            status: newStatus,
            status_reason: reason,
            ...extraPatch,
          ...
          if (
            ...
            Alert.alert(
              "Status Updated",
              `Order marked as ${String(newStatus).replace(/_/g, " ")}\n\nNote: Payment capture will be processed later.`,
              [{ text: "OK" }],
            );
          }
  ...
  /* ===========================
      doUpdate (cluster style) with auto-chat
      =========================== */
  const doUpdate = useCallback(
    async (newStatusRaw, opts = {}, skipUnavailableCheck = false) => {
      ...
      if (newStatus === "CONFIRMED" && currentStatus === "PENDING") {
        const selectedMethod = opts?.deliveryMethod || deliveryChoice;
        if (!selectedMethod) {
          Alert.alert("Error", "No delivery method selected");
          return;
        }
        ...
        await updateSingleStatusLikeCluster({
          ...
          showSuccessAlert: true,
          replacementsForChat: unavailable_changes.replaced || [], // ✅ Send replacements to auto-chat
        });
  ...
  const onSetDeliveryChoice = useCallback(
    async (choice) => {
      ...
      // ✅ For PENDING status and GRAB selection, redirect immediately
      if (nextChoice === "grab" && status === "PENDING") {
        setPendingDeliveryChoice(nextChoice);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["left", "right", "bottom"]}>
      ...
      >
        ...
        {!isScheduledOrder && (
          <>
            {/* ✅ Show DeliveryMethodChooser ONLY at PENDING status */}
            {status === "PENDING" &&
              !isTerminalNegative &&
              !isTerminalSuccess && (
                <View style={{ marginTop: S(16) }}>
                  ...
                  <DeliveryMethodChooser
                    ...
                    isTerminalNegative={isTerminalNegative}
                    isTerminalSuccess={isTerminalSuccess}
                    isSelfSelected={pendingDeliveryChoice === "self"}
                    isGrabSelected={pendingDeliveryChoice === "grab"}
                    sendingGrab={sendingGrabRequest}
                    rideMessage={rideMessage}
                    driverSummaryText={driverSummaryText}
                    driverAccepted={driverAccepted}
                    setDeliveryChoice={(choice) => {
                      setPendingDeliveryChoice(choice);
                      if (choice === "self") {
                        setRideMessage("Self delivery selected");
                      } else {
                        setRideMessage("Tabdhey delivery selected");
                    ...
                    stopGrabLoop={stopGrabLoopForPending}
                    startGrabLoop={startGrabLoopForPending}
            ...
            {/* ✅ At READY/OUT_FOR_DELIVERY - Show driver info for GRAB, nothing for SELF */}
            {(status === "READY" || status === "OUT_FOR_DELIVERY") &&
              !isTerminalNegative &&
              !isTerminalSuccess && (
                <>
                  {/* Show driver status for GRAB orders */}
                  {(isGrabSelected || pendingDeliveryChoice === "grab") &&
                    driverAccepted && (
                      <View
                        style={[
                      ...
                      >
                        ...
                        >
                          {driverSummaryText || "Driver is on the way"}
                        </Text>
                        <Text
                          style={{
                            marginTop: 2,
                        ...
                        >
                          {rideMessage || "Order is out for delivery"}
                        </Text>
                      </View>
                    )}

                  {/* Show waiting message for GRAB without driver */}
                  {(isGrabSelected || pendingDeliveryChoice === "grab") &&
                    !driverAccepted &&
                    !sendingGrabRequest && (
                      <View
                      ...
                      >
                        ...
                        >
                          {rideMessage || "Searching for nearby drivers"}
                        </Text>
                      </View>
                    )}
                </>
            ...
            {/* UpdateStatusActions - Modified for PENDING */}
            {status === "PENDING" ? (
              <View style={{ marginTop: S(16) }}>
                <Pressable
                  onPress={handleAcceptWithSelectedMethod}
                  disabled={updating || !pendingDeliveryChoice}
                  style={[
                    styles.acceptButton,
                    (!pendingDeliveryChoice || updating) && { opacity: 0.5 },
                  ]}
                >
              ...
              /* For non-PENDING statuses - Always show for SELF delivery, hide for GRAB until driver assigned */
              (isSelfSelected || (!isPlatformDelivery && !driverAccepted)) && (
                <View style={{ marginTop: S(12) }}>
                  <UpdateStatusActions
                    ...
                    isSelfSelected={isSelfSelected} // ✅ ADD THIS LINE
                    updating={updating}
                    next={next}
      ...
      {/* ✅ AutoChatRedirect Component */}
      <AutoChatRedirect
        visible={showAutoChat}
        onComplete={(result) => {
          console.log("Auto-chat completed:", result);
          setShowAutoChat(false);
          setPendingReplacements([]);

          if (result.success) {
            Alert.alert(
              "✅ Customer Notified",
              `Replacement details have been sent to the customer via chat.\n\nYou can continue the conversation in the chat window.`,
              [{ text: "OK" }],
            );
          }
        }}
        onError={(error) => {
          console.log("Auto-chat error:", error);
          setShowAutoChat(false);
          setPendingReplacements([]);

          Alert.alert(
            "Order Accepted",
            "The order has been accepted with replacements. Please notify the customer about the changes using the chat button.",
          );
        }}
        order={order}
        replacements={pendingReplacements}
        businessId={businessId || paramBusinessId}
        businessName={order?.business_name || params?.business_name || ""}
